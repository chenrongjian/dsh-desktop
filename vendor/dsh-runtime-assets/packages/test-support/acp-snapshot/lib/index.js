	...
	scenarioSuite("snapshot scenarios", () => {
		for (const scenario of scenarios) it.skipIf(scenarioSkipped(scenario, RECORDING, process.platform, options.hasPwsh))(`snapshot: ${scenario.name} matches the expected outputs`, async ({ expect }) => {
			...
			if (RECORDING && scenario.recorded && scenario.hasModelTurn || REFRESHING && comparesLog) {
				...
				const outputFixtures = stabilizeFixtureMessageIds(REFRESHING ? result.sessionLogs.map((log, index) => scrubSessionSnapshot(portableFixture(stabilizeRefreshLog(log.content, existingFixtures[index], refreshReplacements, ctx)))) : result.sessionLogs.map((log) => scrubSessionSnapshot(portableFixture(log.content))), existingFixtures);
...
//#endregion
export { defineAcpSnapshotSuite, extractSnapshotSpillPaths, launchAcpTestAgent, normalizeSessionLog, normalizeSessionSnapshot, normalizeStdout, refreshFixtureReplacements, runScenario, scrubRequestHeaders, scrubSessionSnapshot, scrubSystemPrompts, scrubToolSchemas, stabilizeFixtureMessageIds, stabilizeRefreshLog, tokenizeSessionFixtureCwd };
