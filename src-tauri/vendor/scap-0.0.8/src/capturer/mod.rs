...
impl Capturer {
    ...
    /// Non-blocking variant: returns the next frame immediately if one is
    /// available, otherwise returns None. Useful for loops that must also
    /// respond to external stop signals without blocking on `rx.recv()`.
    pub fn try_get_next_frame(&self) -> Option<Frame> {
        loop {
